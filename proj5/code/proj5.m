...
detection_params = struct('threshold', 0.1, 'scale_factor', 0.9, 'num_scales', 15);

%% Step 1. Load positive training crops and random negative examples
%YOU CODE 'get_positive_features' and 'get_random_negative_features'

features_pos = get_positive_features( train_path_pos, feature_params );
%features_pos = augment_positive_features( train_path_pos, feature_params );
...
X = X';
s1 = size(features_pos, 1);
s2 = size(features_neg, 1);
y1 = ones(s1,1);
y2 = -ones(s2,1);
Y = [y1;y2];
[w b] = vl_svmtrain(X,Y,lambda);
svm_fn = @(features) features*w + b;

% [classifier, net, tr] = train_net(features_pos, features_neg);
...
% features_hard_neg = get_hard_negative_features(non_face_scn_path, svm_fn, feature_params, detection_params);
% 
% lambda = 0.0001;
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, svm_fn, feature_params, detection_params);
