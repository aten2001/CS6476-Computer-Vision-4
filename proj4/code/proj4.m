...
switch lower(FEATURE)    
    ...
    case 'gist'
        train_image_feats = gist_f(train_image_paths);
        test_image_feats  = gist_f(test_image_paths);
...
%         end
    ...
    case 'bag of sift'
        ...
        if ~exist('vocab_200.mat', 'file')
        %if ~exist('vocab_100.mat', 'file')
        %if ~exist('vocab.mat', 'file')
            fprintf('No existing visual word vocabulary found. Computing one from training images\n')
            vocab_size = 200; %Larger values will work better (to a point) but be slower to compute
            %vocab_size=10000;
            vocab = build_vocabulary(train_image_paths, vocab_size);
            save('vocab_200.mat', 'vocab')
            %save('vocab_100.mat', 'vocab')
            %save('vocab.mat', 'vocab')
        end
        
        % YOU CODE get_bags_of_sifts.m
        train_image_feats = get_bags_of_sifts(train_image_paths);
        %train_image_feats = kernel_codebook_encoding(train_image_paths);
        test_image_feats  = get_bags_of_sifts(test_image_paths);
        %test_image_feats  = kernel_codebook_encoding(test_image_paths);
...
switch lower(CLASSIFIER)    
    ...
    case 'modified svm'
        predicted_categories = my_svm(train_image_feats, train_labels, test_image_feats);
